<?php
	...
	$querysearch="SELECT id, name, address, cp, open, close, capacity, employee, mushalla, park_area, bathroom, id_category, st_x(st_centroid(geom)) as lng, st_y(st_centroid(geom)) as lat, st_distancesphere(ST_GeomFromText('POINT(".$longi." ".$latit.")',-1), geom) as jarak FROM restaurant where st_distancesphere(ST_GeomFromText('POINT(".$longi." ".$latit.")',-1), geom) <= ".$rad.""; 
